...
def generate_deep_report(df, name):
    ...
    <div class="report-box">
        ...
        <br><i>(注：图中绿色/红色三角标记分别代表有效的顶/底分型，是趋势转折的关键信号。)</i>
    </div>
    """
    ...
    <div class="report-box" style="margin-top:10px;">
        ...
        <span class="tech-term">江恩角度线 (Gann Fan)</span> 代表了时间与价格的平衡关系。
        <br>• <b>1x1 线</b>：这是45度生死线。股价若在上方运行，表明处于强势多头区域；若跌破，则意味着走势转弱。
        <br><br>
        <span class="tech-term">斐波那契回撤 (Fibonacci)</span> 利用黄金分割率 (0.618) 预测阻力与支撑。
        <br>• <b>当前位置</b>：股价 ({curr['close']:.2f}) 正逼近斐波那契 <b>{fib_near[0]}</b> 关键位 ({fib_near[1]:.2f})。此处极易发生筹码交换，请密切关注是否出现反转K线。
    </div>
    """
    ...
    <div class="report-box" style="margin-top:10px;">
        ...
        <ul>
            <li><span class="tech-term">MACD (指数平滑异同移动平均线)</span>：
                当前处于 <b>{macd_state}</b> 状态。DIF值为 {curr['DIF']:.2f}。
                <br><i>含义：反映中线趋势的动能，红柱代表多头主导，绿柱代表空头主导。</i>
            </li>
            <li><span class="tech-term">MA (均线系统)</span>：
                MA5({curr['MA5']:.2f}) {"大于" if curr['MA5']>curr['MA20'] else "小于"} MA20({curr['MA20']:.2f})。
                <br><i>含义：MA20是短期生命线，MA60是中期牛熊分界线。</i>
            </li>
            <li><span class="tech-term">BOLL (布林带)</span>：
                股价正运行于 { "中轨上方" if curr['close']>curr['MA20'] else "中轨下方" }。
                <br><i>含义：布林带收口代表变盘在即，开口代表趋势延续。</i>
            </li>
            <li><span class="tech-term">VOL (量能)</span>：
                今日呈现 <b>{vol_state}</b> 态势 (量比 {curr['VolRatio']:.2f})。
                <br><i>含义：量在价先，放量上涨通常更可靠，无量上涨需警惕诱多。</i>
            </li>
        </ul>
    </div>
    """
...
def analyze_score(df):
    c = df.iloc[-1]; p = df.iloc[-2]; score=0; reasons=[]
...
def plot_chart(df, name, gann, fib, chan):
...
with st.spinner("AI 正在生成深度研报..."):
    df = get_data(st.session_state.code, token, days, adj)
...
if df.empty:
    ...
    # 2. 图表
    plot_full_chart(df.tail(days), f"{name} 分析图", gann, fib, chan)
    ...
    s1,s2 = st.columns(2)
    s1.info(f"🛡️ 止损: {sl:.2f}"); s2.info(f"💰 止盈: {tp:.2f}")
