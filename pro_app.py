...
# ==========================================
st.set_page_config(
    page_title="阿尔法量研 Pro V68 (全功能商业版)",
    layout="wide",
    page_icon="🔥",
...
def generate_mock_data(days=365):
    ...
    # 防止 KeyError，补全所有指标列
    df['SpanA'] = df['close'] * 0.9
    df['SpanB'] = df['close'] * 0.8
    df['ADX'] = 25.0
    df['VolRatio'] = 1.0
    return df
...
def generate_deep_report(df, name):
    curr = df.iloc[-1]
    chan_trend = "底分型" if curr['F_Bot'] else "顶分型" if curr['F_Top'] else "中继"
...
with st.sidebar:
    ...
    <div style='text-align: left; margin-bottom: 20px;'>
        ...
        <div class='brand-en'>V68.0 商业完全体</div>
    </div>
...
# 图表
name = get_name(st.session_state.code) # 获取名称
